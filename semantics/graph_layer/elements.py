...
class Element(typing.Generic[PersistentIDType]):
    ...
    def __init__(self, controller: 'interface.BaseController', index: PersistentIDType):
        ...
        # Set self._released to True temporarily in case of an exception while acquiring the reference, since __del__
    ...
    def __eq__(self, other: 'Element') -> bool:
        if self._released:
            raise exceptions.InvalidatedReferenceError(self)
        return type(self) is type(other) and self._controller is other._controller and self._index == other._index

    def __ne__(self, other: 'Element') -> bool:
        if self._released:
            raise exceptions.InvalidatedReferenceError(self)
        return not (type(self) is type(other) and self._controller is other._controller and self._index == other._index)
...
class Role(Element[indices.RoleID]):
    """Roles are GraphElements that serve as a sort of indicator for the expected behavior and usage of each Vertex.

    RoleID is the index type, used to uniquely identify which Role is referred to. RoleData is an internally used data
    class for the attributes of the Role. Role is an *indirect reference* to the RoleData, via the RoleID, and serves as
    the externally accessible interface. Multiple Role instances can refer to the same RoleData if they have the same
    RoleID."""
...
class Vertex(Element[indices.VertexID]):
    """Vertices are GraphElements that can be linked to each other via Edges. Vertices know which Edges are inbound and
    which are outbound. Each Vertex always has an associated Role, which acts as a sort of indicator for the expected
    behavior and usage of the Vertex. A Vertex's Role can never be changed once the Vertex is created.

    VertexID is the index type, used to uniquely identify which Vertex is referred to. VertexData is an internally used
    data class for the attributes of the Vertex. Vertex is an *indirect reference* to the VertexData, via the VertexID,
    and serves as the externally accessible interface. Multiple Vertex instances can refer to the same VertexData if
    they have the same VertexID."""
    ...
    def add_edge_to(self, edge_label: 'Label', sink: 'Vertex') -> 'Edge':
        if self._released:
            raise exceptions.InvalidatedReferenceError(self)
        return Edge(self._controller, self._controller.add_edge(edge_label.index, self._index, sink.index))

    def add_edge_from(self, edge_label: 'Label', source: 'Vertex') -> 'Edge':
        if self._released:
            raise exceptions.InvalidatedReferenceError(self)
        return Edge(self._controller, self._controller.add_edge(edge_label.index, source.index, self._index))
...
class Label(Element[indices.LabelID]):
    """Labels are GraphElements that serve as a sort of indicator for the expected behavior and usage of each Edge.

    LabelID is the index type, used to uniquely identify which Label is referred to. LabelData is an internally used
    data class for the attributes of the Label. A Label instance is an *indirect reference* to the LabelData, via the
    LabelID, and serves as the externally accessible interface. Multiple Label instances can refer to the same LabelData
    if they have the same LabelID."""
...
class Edge(Element[indices.EdgeID]):
    """Edges are GraphElements that link two Vertices together. Edges are always directed. Each Edge always has an
    associated Label, which acts as a sort of indicator for the expected behavior and usage of the Edge. An Edge's
    Label can never be changed once the Edge is created.

    EdgeID is the index type, used to uniquely identify which Edge is referred to. EdgeData is an internally used data
    class for the attributes of the Edge. Edge is an *indirect reference* to the EdgeData, via the EdgeID, and serves as
    the externally accessible interface. Multiple Edge instances can refer to the same EdgeData if they have the same
    EdgeID."""
